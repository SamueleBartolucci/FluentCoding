...
namespace FluentCoding.Test.FluentExtensions.Or.Generics
{
    ...
    public class OrOptn_T_Optn_Func_Bool_Tests
    {
        [Test]
        public void OrOptn_None_None()
         => Optional<string>.None().OrOptn(Optional<string>.None(), () => true)
            .IsNone().Should().BeTrue();


        [Test]
        public void OrOptn_None_Some()
         => Optional<string>.None().OrOptn("some".ToOptional(), () => true)
            .IsSome().Should().BeTrue();


        [Test]
        public void OrOptn_String_Left1()
            => Test.LEFT.ToOptional().OrOptn(Test.RIGHT.ToOptional(), () => false)
                .Do(result => result.IsSome().Should().BeTrue()).Subject.Should().Be(Test.LEFT);

        [Test]
        public void OrOptn_String_Left2()
            => Test.LEFT.ToOptional().OrOptn(Test.GetDefault<string>().ToOptional(), () => false)
                .Do(result => result.IsSome().Should().BeTrue()).Subject.Should().Be(Test.LEFT);

        [Test]
        public void OrOptn_StringEmpty_Right()
            => string.Empty.ToOptional().OrOptn(Test.RIGHT.ToOptional(), () => false)
                .Do(result => result.IsSome().Should().BeTrue()).Subject.Should().Be(string.Empty);

        [Test]
        public void OrOptn_StringSpaces_Right()
            => " ".ToOptional().OrOptn(Test.RIGHT.ToOptional(), () => false)
        ...
        public void OrOptn_Object_Left()
            => Test.NewTLeft
                .ToOptional().OrOptn(Test.NewTRight.ToOptional(), () => false)
        ...
        public void OrOptn_Null_RightPriority_Right()
            => Test.GetDefault<TType>().ToOptional().OrOptn(Test.NewTRight.ToOptional(), () => true)
                .Do(result => result.IsSome().Should().BeTrue()).Subject.Should().BeEquivalentTo(Test.NewTRight);

        [Test]
        public void OrOptn_Null_LeftPriority_Right()
          => Test.GetDefault<TType>().ToOptional().OrOptn(Test.NewTRight.ToOptional(), () => false)
              .Do(result => result.IsSome().Should().BeTrue()).Subject.Should().BeEquivalentTo(Test.NewTRight);

        [Test]
        public void OrOptn_Object_RightPriority_Right()
            => Test.NewTLeft.ToOptional().OrOptn(Test.NewTRight.ToOptional(), () => true)
                .Do(result => result.IsSome().Should().BeTrue()).Subject.Should().BeEquivalentTo(Test.NewTRight);
    }
}
