...
namespace FluentCoding
{
    public static class Equals_T
    {
        ...
        public static bool EqualsToAny<T>(this T _, Func<T, T, bool> equalityComparison, params T[] domainsToCompare)
            => _.IsNullOrDefault() ? false : domainsToCompare.Any(domainValue => _.EqualsTo(domainValue, equalityComparison));//equalityComparison(_, domainValue));
        ...
        public static bool EquivalentToAny<T, K>(this T _, Func<T, K, bool> equivalentComparison, params K[] domainsToCompare)
            => _.IsNullOrDefault() ? false : domainsToCompare.Any(domainValue => _.EquivalentTo(domainValue, equivalentComparison));//equalityComparison(_, domainValue));
