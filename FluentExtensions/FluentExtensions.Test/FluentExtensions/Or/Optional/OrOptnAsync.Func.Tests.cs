...
namespace FluentCoding.Test.FluentExtensions.Or.Generics
{
    ...
    public class OrOptnAsync_Func_Tests
    {

        [Test]
        public void OrOptnAsync_String_Left1()
            => Test.LEFT.ToOptional().ToTask().OrOptnAsync(Test.RIGHT, () => false)
                .Result.Should().Be(Test.LEFT);

        [Test]
        public void OrOptnAsync_String_Left2()
            => Test.LEFT.ToOptional().ToTask().OrOptnAsync(null, () => false)
                .Result.Should().Be(Test.LEFT);

        [Test]
        public void OrOptnAsync_StringEmpty_Right()
            => string.Empty.ToOptional().ToTask().OrOptnAsync(Test.RIGHT, () => false)
                .Result.Should().Be(string.Empty);

        [Test]
        public void OrOptnAsync_StringSpaces_Right()
            => " ".ToOptional().ToTask().OrOptnAsync(Test.RIGHT, () => false)
                .Result.Should().Be(" ");
        ...
        public void OrOptnAsync_Object_Left()
            => Test.NewTLeft
                .ToOptional().ToTask().OrOptnAsync(Test.NewTRight, () => false)
        ...
        public void OrOptnAsync_Null_LeftPriority_Right()
           => Test.GetDefault<TType>().ToOptional().ToTask().OrOptnAsync(Test.NewTRight, () => false)
               .Result.Should().BeEquivalentTo(Test.NewTRight);
